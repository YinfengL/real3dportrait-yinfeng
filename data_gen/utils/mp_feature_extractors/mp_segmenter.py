...
def scatter_np(condition_img, classSeg=6):
    """
    将类别索引图转换为 One-Hot 编码。
    强制处理输入维度，确保为 (B, C, H, W)
    """
    ...
    if condition_img.ndim == 2:
        # [H, W] -> [1, 1, H, W]
        condition_img = condition_img[None, None, :, :]
    elif condition_img.ndim == 3:
        # 可能是 [B, H, W] 或 [H, W, C]
        # 假设如果是 [H, W, 1] 这种，需要调整
        if condition_img.shape[-1] == 1:
             condition_img = condition_img[:, :, 0][None, None, :, :]
        else:
             # 假设是 [B, H, W]
             condition_img = condition_img[:, None, :, :]
    elif condition_img.ndim > 4:
        # 如果维度太多，取前两个作为 B, C? 或者直接报错提示
        # 这里做一个安全的切片，只取前 4 维
        condition_img = condition_img.reshape(condition_img.shape[:4])
        
    # 再次确认维度
    if condition_img.ndim != 4:
        raise ValueError(f"scatter_np input must be 4D after processing, got {condition_img.shape}")

    batch, c, height, width = condition_img.shape
    
    # 创建 one-hot 输出
    input_label = np.zeros([batch, classSeg, height, width], dtype=np.int_)
    
    # 使用 put_along_axis 进行散射
    # condition_img 的值必须在 0 到 classSeg-1 之间
    np.put_along_axis(input_label, condition_img, 1, axis=1)
...
def extract_background(img_lst, segmap_lst=None):
    ...
    for idx, img in enumerate(img_lst):
        if segmap_lst is not None:
            ...
            segmap = seg_model.cal_seg_map(img)
...
class MediapipeSegmenter:
    def __init__(self):
        ...
        self.video_options = vision.ImageSegmenterOptions(
            base_options=base_options, running_mode=vision.RunningMode.VIDEO, output_category_mask=True
        )
    ...
    def multiprocess_cal_seg_map_for_a_video(self, imgs, num_workers=4):
        ...
        for (i, res) in multiprocess_run_tqdm(
            job_cal_seg_map_for_image, args=img_lst, num_workers=num_workers, desc='extracting from a video in multi-process'
        ):
    ...
    def _cal_seg_map(self, img, segmenter=None, return_onehot_mask=True):
        ...
        # 1. 获取原始 category_mask [H, W]
        raw_segmap = out.category_mask.numpy_view().copy()
        
        # 确保 raw_segmap 是纯 numpy 数组且是 2 维
        if not isinstance(raw_segmap, np.ndarray):
            raw_segmap = np.array(raw_segmap)
        if raw_segmap.ndim != 2:
            # 如果 MP 返回了多余维度，压平它
            raw_segmap = raw_segmap.squeeze()
            if raw_segmap.ndim != 2:
                raise ValueError(f"Segmentation mask shape invalid after squeeze: {raw_segmap.shape}")

        # 2. 获取 one-hot mask
        one_hot_segmap = None
        if return_onehot_mask:
            # 【关键修改】直接传 raw_segmap (2D)，让 scatter_np 自己去处理维度
            # 不要在这里手动加 [None, None]，避免维度叠加错误
            one_hot_segmap = scatter_np(raw_segmap, classSeg=6)[0]  # 取出 batch 的第 0 个
        ...
        # 返回 4 个值
        return one_hot_segmap, raw_segmap, dummy_confidences, dummy_categories
    ...
    def _seg_out_img_with_segmap(self, img, segmap, mode='head'):
        ...
        if mode == 'head':
            ...
            selected_mask = np.ones((1, img.shape[0], img.shape[1]), dtype=bool)
        else:
    ...
    def _seg_out_img(self, img, segmenter=None, mode='head'):
        ...
        """
        segmenter = vision.ImageSegmenter.create_from_options(self.options) if segmenter is None else segmenter
        segmap, _, _, _ = self._cal_seg_map(img, segmenter=segmenter, return_onehot_mask=True)

        return self._seg_out_img_with_segmap(img, segmap, mode=mode)

    def cal_seg_map(self, img):
        """
        public api for one image
        img: [H, W, 3], 0~255
        return: [6, H, W]
        """
        # cal_seg_map 是公共 API，通常只应该返回 mask，所以这里也要解包只返回第一个
        segmap, _, _, _ = self._cal_seg_map(img, return_onehot_mask=True)
        return segmap
