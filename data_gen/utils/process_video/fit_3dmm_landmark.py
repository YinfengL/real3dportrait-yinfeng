...
def cal_lap_loss(in_tensor):
    ...
    lap_kernel = torch.Tensor((-0.5, 1.0, -0.5)).reshape([1, 1, 3]).float().to(in_tensor.device)
...
def fit_3dmm_for_a_video(
    ...
    """
    Return:
        coeff_dict: dict with keys ['id', 'exp', 'euler', 'trans']
        or None if failed
    """
    ...
    if id_mode == 'global':
        ...
        LAMBDA_REG_EXP = 0.6
        LAMBDA_REG_LAP = 1.0
        LAMBDA_REG_VEL_ID = 0.0
        LAMBDA_REG_VEL_EXP = 0.0
    else:
        LAMBDA_REG_ID = 0.3
        LAMBDA_REG_EXP = 0.05
        LAMBDA_REG_LAP = 1.0
    ...
    try:
        ...
        if os.path.exists(lm_name):
            ...
            try:
                ...
                import traceback
                print("[ERROR] failed while extracting landmarks from video:", flush=True)
        ...
        for i in range(int((num_frames - 1) / batch_size + 1)):
            ...
            sel_euler_angle = euler_angle.new_zeros((batch_size, 3), requires_grad=True)
            ...
            if id_mode == 'global':
                ...
                optimizer_cur_batch = torch.optim.Adam([sel_exp_para, sel_euler_angle, sel_trans], lr=0.005)
            else:
                set_requires_grad([sel_id_para, sel_exp_para, sel_euler_angle, sel_trans])
                optimizer_cur_batch = torch.optim.Adam([sel_id_para, sel_exp_para, sel_euler_angle, sel_trans], lr=0.005)

            for j in range(50):
                ret = {}
                proj_geo = face_model.compute_for_landmark_fit(
                    sel_id_para, sel_exp_para, sel_euler_angle, sel_trans, ret)
                loss_lan = cal_lan_loss_fn(proj_geo[:, :, :2], lms[sel_ids].detach())
        ...
        if debug:
            ...
            xy_camera_visualizer = CameraPoseVisualizer(
                ...
                zlim=[extrinsic[:, 2, 3].min().item() - 0.5, extrinsic[:, 2, 3].max().item() + 0.5],
                view_mode='xy'
            )
            ...
            if id_mode == 'global':
                id_para_vis = id_para.repeat([exp_para.shape[0], 1])
            else:
                id_para_vis = id_para

            proj_geo = face_model.compute_for_landmark_fit(id_para_vis, exp_para, euler_angle, trans)
            lm68s = proj_geo[:, :, :2].detach().cpu().numpy()
            lm68s = lm68s * img_scale_factor
            lms_vis = lms * img_scale_factor
            lm68s[..., 1] = img_h - lm68s[..., 1]
            lms_vis[..., 1] = img_h - lms_vis[..., 1]
            ...
            for i in tqdm.trange(min(250, len(frames)), desc=f'rendering debug video to {debug_name}..'):
                ...
                img = draw_axes(
                    img,
                    euler_angle[i, 0].item(),
                    euler_angle[i, 1].item(),
                    euler_angle[i, 2].item(),
                    lm68s[i][4][0].item(),
                    lm68s[i, 4][1].item(),
                    size=50
                )
                ...
                for lm in lm68s[i]:
                    img = cv2.circle(img, tuple(lm), 1, (0, 0, 255), thickness=-1)
                for gt_lm in lms_vis[i]:
                    img2 = cv2.circle(img2, tuple(gt_lm.cpu().numpy().astype(int)), 2, gt_lm_color, thickness=1)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--id_mode", default='global', type=str)
    parser.add_argument("--keypoint_mode", default='mediapipe', type=str)
    parser.add_argument("--large_yaw_threshold", default=9999999.9, type=float)
    ...
    for i in tqdm.trange(len(vid_names), desc=f"process {process_id}: fitting 3dmm ..."):
        ...
        try:
            ...
            ret = fit_3dmm_for_a_video(
                img_name,
                is_person_specific_data,
                args.id_mode,
                args.debug,
                keypoint_mode=args.keypoint_mode,
