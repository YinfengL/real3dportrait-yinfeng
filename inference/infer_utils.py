...
def mirror_index(index, len_seq):
    """
    get mirror index when indexing a sequence and the index is larger than len_seq
    args:
        index: int
        len_pose: int
    return:
...
def smooth_features_xd(in_tensor, kernel_size=7):
    ...
    pad = (kernel_size - 1) // 2
    in_tensor = torch.cat(
        [torch.flip(in_tensor[0:pad], dims=[0]), in_tensor, torch.flip(in_tensor[t - pad:t], dims=[0])],
        dim=0
    )
    if ndim == 2:  # tc
        _, c = in_tensor.shape
...
def save_wav16k(audio_name):
    ...
    assert audio_name.endswith(supported_types), (
        f"Now we only support {','.join(supported_types)} as audio source!"
    )
